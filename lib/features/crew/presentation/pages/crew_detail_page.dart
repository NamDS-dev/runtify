                      ...
                      return Container(
                        ...
                        child: Row(
                          children: [
                            ...
                            // 퇴출 버튼 (리더에게만 표시, 리더 본인 제외)
                            if (isLeader && !isCrewLeader) ...[
                              const SizedBox(width: 12),
                              GestureDetector(
                                onTap: () => _showKickDialog(
                                  context, ref, currentCrew.id, member.id, member.name,
                                ),
                                child: const Icon(
                                  Icons.close,
                                  size: 18,
...
class _ActionButton extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    if (isMember) {
      ...
      return SizedBox(
        width: double.infinity,
        child: ElevatedButton(
          onPressed: (isFull || isLoading) ? null : () => _joinCrew(context),
          style: ElevatedButton.styleFrom(
            padding: const EdgeInsets.symmetric(vertical: 14),
  ...
  Future<void> _joinCrew(BuildContext context) async {
    ...
    if (success) {
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(content: Text('가입 실패: $error'), backgroundColor: Colors.red),
      );
    }
  }
