...
| 명령어 | 역할 |
|--------|------|
| `/planning` | 1차 기획 (인터뷰 → 명세) |
| `/planning-review` | 기획 레드팀 (4렌즈 비판적 검토) |
...
1단계 기획 (/planning)
  인터뷰(~10문 3블록) → 명세 v1
  → 레드팀 검토 (/planning-review, 4렌즈: 러너현실·경쟁·가설ROI·UX) → 의견서
  → 기획자 반영 → 명세 v2 (+ v1→v2 변경점 + 미해결 논쟁)
  ━━ 게이트 1: 사용자 승인 (미해결 논쟁은 사용자가 판정) ━━
