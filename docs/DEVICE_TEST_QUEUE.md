...
- [ ] **🔴 [버그] Android 러닝 중 GPS 거리 0km — 알림 권한 + onError 재구독 픽스 (2026-05-03 수정, 가장 핵심)**
  ...
  - **사전 준비 (Mac에서, 출시 전이라 Play Store 설치 불가)**:
    ```bash
    flutter build apk --release
    flutter install --release -d <device-id>
    ```
    USB 분리 후 외출. "앱 재설치" 대신 아래 방법으로 첫 실행 시나리오 재현:
    - **방법 A (간단)**: 설정 → 앱 → Runtify → 권한 → 위치/알림 모두 거부
    - **방법 B (깨끗)**: 설정 → 앱 → Runtify → 저장소 → 데이터 삭제 (로그인도 풀림)
  - 재현/검증 절차:
    1. **첫 실행 시나리오 (가장 중요)**: 데이터 삭제 또는 권한 전체 거부 → 앱 실행 → 로그인 → 러닝 시작 → 권한 다이얼로그(GPS, 알림) 모두 허용 → 야외 100m+ 실제 이동 → 거리 정상 카운트 확인
    2. **알림 거부 시나리오**: 권한 초기화 → 앱 실행 → 로그인 → 러닝 시작 → GPS는 허용, **알림은 거부** → 야외 100m+ 이동 → 거리 정상 카운트 (foreground 알림은 안 떠도 GPS는 살아 있어야 함)
    3. **이미 권한 부여된 시나리오**: 위 1번 후 앱 종료 → 재시작 → 러닝 시작 → 권한 다이얼로그 없이 즉시 GPS 시작 + 거리 카운트
