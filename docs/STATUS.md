...
3. **남은 야간 큐 결정**: 워치 미연결 안내, 일시정지/자동 일시정지(GPS 라이프사이클), Deep Link, 인증 잔여 가드, 회원 탈퇴 (180분)
4. **신규 🟡 결정**: timeout 정책 / 닉네임 사후 변경 / wakelock_plus (FEATURE_PLAN 🟡 참조)
5. **계정 탈퇴 야간 큐 (Flutter 측)** — POLICY § 4 기반, 약 180분
6. **iOS 백그라운드 복귀 크래시 디버깅** — 실기기/TestFlight 필요
