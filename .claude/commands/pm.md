...
## Step 2: 1단계 기획 + 레드팀 검토 (게이트 1)

기획은 **1차 기획 → 레드팀 검토 → 반영(v2)** 의 내부 루프를 거친 뒤 게이트 1.

1. **1차 기획** — 서브에이전트(`general-purpose`)에게 `.claude/commands/planning.md` Step 1~3 실행 지시 → 명세 v1
   - 단, 인터뷰(Q1=B 3블록)는 **PM이 사용자와 직접 `AskUserQuestion`으로 중계** (서브에이전트는 답변을 받아 명세화)
2. **레드팀 검토** — 별도 서브에이전트에게 `.claude/commands/planning-review.md` 실행 지시 (입력: 명세 v1) → 4렌즈 검토 의견서
3. **반영(v2)** — 1차 기획 서브에이전트에게 검토 의견 전달 → 명세 v2 + `v1→v2 변경점` + `미해결 논쟁`

```
게이트 1 (PM이 AskUserQuestion):
  ① 명세 v2  ② v1→v2 변경점  ③ 미해결 논쟁(레드팀 vs 기획자 — 사용자 판정)
  승인 → Step 3 / 수정 → 기획~레드팀 재실행 후 재게이트 / 중단 → 종료
```

> Q2=A: 레드팀 검토는 **모든 기획 필수**. 검토 없이 게이트 1 진입 금지.
> Q3=A: 미해결 논쟁은 임의 결정 금지 — 양쪽 주장 그대로 사용자에게.
